  ...
  static bool evaluate(PathStatistics &stats,
    ...
    if (path.getStateCount() < 2) {
      ...
      if (global::settings.evaluate_clearing &&
        ...
        stats.mean_clearing_distance = stat2::mean(clearings);
        stats.median_clearing_distance = stat2::median(clearings);
        stats.min_clearing_distance = stat2::min(clearings);
        stats.max_clearing_distance = stat2::max(clearings);
      }
