...
#ifdef G1_AVAILABLE
#include "kimm_path_planner/steer_functions/G1Clothoid/ClothoidSteering.hpp"
#endif
...
void PlannerSettings::GlobalSettings::EnvironmentSettings::CollisionSettings::
  ...
  if (collision_model == robot::ROBOT_POLYGON) {
    ...
    robot_shape.value().scale(global::settings.env.polygon.scaling * 3);
    ...
    OMPL_INFORM("\tCenter: [%.2f %.2f %.2f] ",
                robot_shape.value().centroid().x, robot_shape.value().centroid().y, global::settings.env.polygon.scaling);
