...
void POSQStateSpace::initialize() { state_space_ = new POSQ(); }
...
void POSQStateSpace::interpolate(const ompl::base::State *from,
                                 const ompl::base::State *to, const double t,
...
void POSQStateSpace::interpolate(const ompl::base::State *from,
                                 const ompl::base::State *to, const double t,
                                 bool &firstTime, vector<UnicycleState> &states,
                                 ompl::base::State *state) const {
  if (firstTime) {
    if (t >= 1.) {
      if (to != state)
...
void POSQStateSpace::interpolate(const ompl::base::State *from,
                                 const vector<UnicycleState> &states, double t,
                                 ompl::base::State *state) const {
  ...
  //    OMPL_DEBUG("Time: %f, x: %f  y:%f  z: %f ", t, result.x_, result.y_,
  //    result.yaw_);
}
...
bool POSQMotionValidator::checkMotion(
  ...
  if (nd > 1) {
    ...
    for (int j = 1; j < nd; ++j) {
      stateSpace_->interpolate(s1, s2, (double)j / (double)nd, firstTime,
                               states, test);
      if (!si_->isValid(test)) {
        lastValid.second = (double)(j - 1) / (double)nd;
        if (lastValid.first != nullptr)
          stateSpace_->interpolate(s1, s2, lastValid.second, firstTime, states,
                                   lastValid.first);
        result = false;
        break;
      }
...
bool POSQMotionValidator::checkMotion(const ompl::base::State *s1,
  ...
  if (nd >= 2) {
    ...
    /* repeatedly subdivide the path segment in the middle (and check the
     * middle) */
    while (!pos.empty()) {
      std::pair<int, int> x = pos.front();

      int mid = (x.first + x.second) / 2;
      stateSpace_->interpolate(s1, s2, (double)mid / (double)nd, firstTime,
                               states, test);
